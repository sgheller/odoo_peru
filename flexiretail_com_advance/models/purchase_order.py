...
class PurchaseOrder(models.Model):
    ...
    is_bill_created = fields.Boolean(string='Bill Created', compute='_check_created_bill')

    @api.depends('order_line')
    def _check_created_bill(self):
        for each in self:
            order_line = each.order_line.filtered(lambda l: l.product_qty != l.qty_invoiced)
            if not order_line:
                each.is_bill_created = True
            else:
                each.is_bill_created = False
