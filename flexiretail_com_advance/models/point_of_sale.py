...
class PosConfig(models.Model):
    ...
    enable_create_product = fields.Boolean(string="Add Product")
...
class pos_order(models.Model):
    ...
    def _action_create_invoice_line(self, line=False, invoice_id=False):
        ...
        inv_line = {
            'line_note': line.line_note if line.line_note else '',
    ...
    def create_from_ui(self, orders):
        ...
        for tmp_order in orders_to_save:
            ...
            if to_invoice and not tmp_order.get('data').get('old_order_id'):
    ...
    def _process_order(self, order):
        ...
        if not order.get('draft_order') and draft_order_id:
            ...
            if order_obj and order_obj.invoice_id:

                for payments in order['statement_ids']:
                    order_obj.add_payment(self._payment_fields(payments[2]))

                session = self.env['pos.session'].browse(order['pos_session_id'])
                if not order['pos_session_id'] == order_obj.session_id.id and order_obj.invoice_id:
                # if order_obj.invoice_id:
                
                    invoice_id = order_obj.invoice_id.id
                    account_payment = self.env['account.payment']
                    for payments in order['statement_ids']:
                        account_journal_obj = self.env['account.journal'].browse(payments[2].get('journal_id'))
                        ...
                        payment_obj = account_payment.create({
                            ...
                            'partner_id': order_obj.invoice_id.partner_id.id,
    ...
    def _order_fields(self, ui_order):
        ...
        res.update({
            ...
            'ret_o_id': ui_order.get('ret_o_id') or False,
            'parent_return_order': ui_order.get('parent_return_order', ''),
            'return_seq': ui_order.get('return_seq', ''),
            'delivery_destinatario': ui_order.get('delivery_destinatario') or False,
    ...
    ret_o_id = fields.Many2one("pos.order")
    rounding_option = fields.Char("Rounding Option")
    rounding = fields.Float(string='Rounding', digits=0)
    delivery_destinatario = fields.Char("Delivery Destinatario")
