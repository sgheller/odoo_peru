odoo.define('flexiretail_com_advance.screens', function (require) {
    ...
    screens.define_action_button({
        ...
        'condition': function(){
            return this.pos.config.pos_sale_order || this.pos.get_order().get_edit_quotation();
        },
    });
...
//            //brand Search
    ...
    screens.ClientListScreenWidget.include({
        ...
        display_client_details: function(visibility,partner,clickpos){
            ...
            if(visibility == "edit"){
                if(self.edit_mode){
                    $("input.detail.doc_number").prop('disabled', true);
                } else {
                    $("input.detail.doc_number").prop('disabled', false);
                }
...
//			}
            ...
            if(self.pos.config.enable_delivery_charges){
                var delivery_destinatario = order.get_delivery_destinatario();
...
//                      }
        ...
        show: function() {
            ...
            $("input#txt_destinatario").focus(function() {
                $('body').off('keypress', self.keyboard_handler);
                $('body').off('keydown',self.keyboard_keydown_handler);
                /*window.document.body.removeEventListener('keypress',self.keyboard_handler);
                window.document.body.removeEventListener('keydown',self.keyboard_keydown_handler);*/
            });
            $("input#txt_destinatario").focusout(function() {
                $('body').off('keypress', self.keyboard_handler).keypress(self.keyboard_handler);
                $('body').off('keydown',self.keyboard_keydown_handler).keydown(self.keyboard_keydown_handler);
                /*window.document.body.addEventListener('keypress',self.keyboard_handler);
                window.document.body.addEventListener('keydown',self.keyboard_keydown_handler);*/
            ...
            if(order.get_is_delivery()){
                ...
                if(order.get_delivery_destinatario()){
                    $('#txt_destinatario').val(order.get_delivery_destinatario());
                }
...
//                }
                ...
                if(self.pos.config.enable_delivery_charges){
                    ...
                    var delivery_destinatario = order.get_delivery_destinatario();
                    if(!delivery_destinatario){
                        delivery_destinatario = $('#txt_destinatario').val();
                        if(delivery_destinatario){
                            order.set_delivery_destinatario(delivery_destinatario);
                        }
                    }
                    var date = order.get_delivery_date();
                    var is_deliver = order.get_is_delivery();
    ...
    screens.ProductScreenWidget.include({
        start: function(){
            ...
            $('#leftmenubtn1').click(function(){
                if($('#leftmenubtn1').hasClass('toggled')){
                    $('#leftmenubtn1').removeClass('toggled');
                } else {
                    $('#leftmenubtn1').addClass('toggled');
                }
                self.call_leftbar_menu();
            });
        },
        create_draft_order: function(){
            ...
            if(self.pos.config.enable_delivery_charges){
                ...
                if(is_deliver && (!delivery_destinatario || !date || !time || !address || !delivery_user_id)){
                    self.pos.db.notification('danger',_t('Delivery information required to validate order!'));
...
//		                	order.set_loyalty_earned_amount(0.00);
        ...
        show: function(){
            ...
            _.each(self.pos.users, function(user){
                self.pos.db.res_user_data[user.id] = user;
            });
            if(this.pos.get_order().get_salesman_id()){
                $('.sale_person_a').css('color','#fff !important');
                $('.sale_person_a').prop('title',self.pos.db.get_user_by_id(this.pos.get_order().get_salesman_id()).name);
            }else{
                $('.sale_person_a').css('color','#C5AFAF !important');
                $('.sale_person_a').prop('title','Sales Person');
            }
            this._super();
...
*********************************************************** */
    ...
    var ReturnOrderListScreenWidget = screens.ScreenWidget.extend({
        template: 'ReturnOrderListScreenWidget',
        init: function(parent, options){
            var self = this;
            this._super(parent, options);
            if(this.pos.config.iface_vkeyboard && self.chrome.widget.keyboard){
        ...
        events: {
            ...
            'click .searchbox .search-clear': 'clear_search',
            'click #print_order': 'click_reprint',
            'click #return_pos_order': 'click_return_order',
        },
        filter:"invoiced",
        date: "all",
        get_orders: function(){
        ...
        start: function(){
            var self = this;
            this._super();
            $('span.return_order_date_filter input#return_datepicker').datepicker({
                dateFormat: 'yy-mm-dd',
                autoclose: true,
                closeText: 'Clear',
                showButtonPanel: true,
        ...
        click_return_order: function(event){
            var self = this;
            self.line = [];
            var selectedOrder = this.pos.get_order();
            var order_id = parseInt($(event.currentTarget).data('id'));
            ...
            if (result) {
                ...
                return rpc.query(orderline_params, {async: false}).then(function(res){
                    ...
                    if(res && res.length > 0){
                        ...
                        selectedOrder.destroy();
                        var selectedOrder = self.pos.get_order();
                        if(result.state == 'draft' || result.state == 'cancel'){
                            return self.pos.db.notification('danger',_t('Sorry, You can not return unpaid/cancel order'));
                        }
                        selectedOrder.set_ret_o_id(result.id);
                        selectedOrder.set_ret_o_ref(result.pos_reference);
                        selectedOrder.set_ret_o_inv_origin(result.number);
                        if(result.partner_id && result.partner_id[0]){
                            var partner = self.pos.db.get_partner_by_id(result.partner_id[0])
                            selectedOrder.set_client(partner);
                        } else{
                        ...
                        self.lines = lines;
                        self.gui.show_popup('pos_return_order',{'lines':self.lines,'line':self.line});
                    } else {
        ...
        check_filters: function(orders){
            ...
            if(self.filter !== "" && self.filter !== "all" && self.filter !== 'reserved'){
                filtered_orders = $.grep(orders,function(order){
                    return order.state === self.filter && !order.back_order && !order.reserved;
                });
            }
            return filtered_orders || orders;
        },
        ...
        render_list: function(orders){
            var self = this;
            if(orders){
                var contents = this.$el[0].querySelector('.return-order-list-contents');
                contents.innerHTML = "";
                var temp = [];
                orders = self.check_filters(orders);
                if(self.date !== "" && self.date !== "all"){
                ...
                for(var i = 0, len = Math.min(orders.length,1000); i < len; i++){
                    var order = orders[i];
                    order.amount_total = parseFloat(order.amount_total).toFixed(2);
                    var return_line_html = QWeb.render('ReturnOrderListLine',{widget: this, order:order});
                    var return_line = document.createElement('tbody');
                    return_line.innerHTML = return_line_html;
                    return_line = return_line.childNodes[1];
                    contents.appendChild(return_line);
                }
            }
        },
        reload_orders: function(){
    ...
    gui.define_screen({name:'return_order_screen', widget: ReturnOrderListScreenWidget});
    ...
    return {
        ...
        ReturnOrderListScreenWidget:ReturnOrderListScreenWidget,
