    ...
    var PosReturnOrderOption = PopupWidget.extend({
        ...
        show: function(options){
            ...
            $('#choice_with_receipt').click(function(){
                self.gui.close_popup();
                self.gui.show_screen('return_order_screen');
...
//    Credit Debit Stmt Popup
         ...
         click_confirm: function(){
             ...
             rpc.query(params, {async: false}).then(function(vals){
                 if(vals){
                    ...
                    if($(".pay_due_amount_credit_debit").prop('checked')){
    ...
    var SalePersonListPopupWidget = PopupWidget.extend({
        template: 'SalePersonListPopupWidget',
        show: function(options){
            var self = this;
            this.renderElement();

            if(self.pos.get_order().get_salesman_id()){
                    $('.set_sales_person').text('Deselect');
                    $('.set_sales_person').addClass('deselect');
            }else{
                $('.set_sales_person').css('display','none');
            }
            this.$('.fa-times').unbind('click').click(function(){
                self.pos.gui.close_popup();
            });

           this.$('.set_sales_person').unbind('click').click(function(){
                if($('.set_sales_person').hasClass('deselect')){
                    self.pos.get_order().set_salesman_id(false);
                    $('.sale_person_a').css('color','#C5AFAF !important');
                    $('.sale_person_a').prop('title','Sales Person');
                }else{
                    self.pos.get_order().set_salesman_id(self.selected_sales_person_detail['selected_sale_person_id']);
                    self.pos.gui.close_popup();
                    $('.sale_person_a').css('color','#fff !important');
                    var user_name = self.pos.db.get_user_by_id(self.selected_sales_person_detail['selected_sale_person_id']);
                    $('.sale_person_a').prop('title',user_name.name);

                }
           });
           this.$('.sales_person_table_details').delegate('.sales_person_tr','click',function(event){
                var selected_sale_person_id = $(this).data('id');
                var selected_sale_person_name = $(this).data('name');
                self.selected_sales_person_detail = {'selected_sale_person_id':selected_sale_person_id,'selected_sale_person_name':selected_sale_person_name}
                if ( $(this).hasClass('highlight') ){
                    $(this).removeClass('highlight');
                    $('.set_sales_person').css('display','none');
                }else{
                    $('.sales_person_table tr.highlight').removeClass('highlight');
                    $(this).addClass('highlight');
                    $('.set_sales_person').css('display','block');
                    if(self.pos.get_order().get_salesman_id() == selected_sale_person_id){
                        $('.set_sales_person').text('Deselect')
                        $('.set_sales_person').addClass('deselect')
                    }else{
                        $('.set_sales_person').text('Select')
                        $('.set_sales_person').removeClass('deselect')
                    }
    ...
    gui.define_popup({name:'sale_person_list', widget: SalePersonListPopupWidget});
