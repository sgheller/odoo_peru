odoo.define('flexiretail_com_advance.db', function (require) {
	"use strict";
...
//            Sale order extension
        ...
        get_user_by_id: function(user_id){
            return this.res_user_data[user_id]
        },
...
//                if(!product.is_packaging){
        ...
        add_orders: function(orders){
            ...
            {                              
                ...
                // 2. collect orders ids
                if(!_partners_ids.includes(order.partner_id[0]))
                    _partners_ids.push(order.partner_id[0])
            ...
            if (updated_count) {
                ...
                // 3. get all partners doc_number [id,doc_number]
                var _all_partners = this._get_all_orders_partners(_partners_ids);
                console.log("ALL PARTNERS")
                console.log(_partners_ids);
                console.log(_all_partners);
                
                // If there were updates, we need to completely 
                this.order_search_string = "";
                for (var id in this.order_by_id) {
                    var order = this.order_by_id[id];
                    this.order_search_string += this._all_order_search_string(order, _all_partners);
                }
            }
            console.log(this.order_search_string)
            return updated_count;
        },

        _get_all_orders_partners: function(partners_ids)
        {
                var partners = null;
                var params =   {
                                    model: 'res.partner',
                ...
                rpc.query(params, {async: false}).then(function(partners_response)
                {
                    console.log('partners_response');
                    console.log(partners_response);
        ...
        _all_order_search_string: function(order, partners){
            var str =  order.name;
            ...
            var doc_number = false;
            for (var i=0;i<partners.length;i++)
            {
                if(order.partner_id[0] == partners[i][0])
                {
                    doc_number = partners[i][1];
                }
            }    
            if(doc_number)
               str += '|' + doc_number;
            ...
            if(order.partner_id.length > 0){
                ...
                rpc.query(params, {async: false}).then(function(partner){
                    if(partner && partner.length > 0){
                        if(partner && partner[0] && partner[0].doc_number){
                            str += '|' + partner[0].doc_number;
                        }
                    }
                });
